...
file = open("PPVAppendixFigures.tex","w")
...
for exp in range(1,28):
	...
	for name in filenames:
		if name.replace(' ',' ')[len(name)-4:] == '.pdf':
			...
			figurename = '\t\t\includegraphics[height=3.05in]{0_Images/Results_Charts/Exp_' + str(exp) + '_Charts/' + name + '}\n'
			file.write(figurename)
			figureTitle = re.sub(r"(?<=\w)([1-9])", r" \1", re.sub(r"(?<=\w)([A-Z])", r" \1", name.replace(' ',' ')[:-4]))
